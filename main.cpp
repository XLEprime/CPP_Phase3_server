...
//             }
//             QJsonObject info;
//             info.insert("username", args[1]);
//             info.insert("password", args[2]);
//             info.insert("type", CUSTOMER);
//             info.insert("name", args[3]);
//             info.insert("phonenumber", args[4]);
//             info.insert("address", args[5]);
//             QString ret = userManage.registerUser(token.toObject(), info);
//             if (ret.isEmpty())
...
//             QString ret = userManage.deliveryItem(token.toObject(), args[1].toInt());
//             if (ret.isEmpty())
//                 qInfo()
//                     << "运送成功";
//             else
...
//                 continue;
//             }
//             if (!args[1].toInt(&ok) || !ok) //超过int范围也会失败
//             {
//                 qInfo() << "单次余额改变量是不超过1000000000的整数";
//                 continue;
//             }
...
//             int cost;
//             QString ret = userManage.sendItem(token.toObject(), info, cost);
//             if (ret.isEmpty())
//                 qInfo() << "快递发送成功，共花费" << cost << "元";
//             else
//                 qInfo() << "快递发送失败" << ret;
//         }
...
//                 continue;
//             }
//             QString ret = userManage.receiveItem(token.toObject(), args[1].toInt());
//             if (ret.isEmpty())
//                 qInfo() << "物品接收成功";
//             else
//                 qInfo() << "物品接收失败" << ret;
